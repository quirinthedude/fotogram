...
document.addEventListener('keydown', function (downKey) {
    ...
    // nur wenn der Dialog offen weiter
    if (downKey.repeat) return;
    // falls jmd zu lange auf der Taste bleibt ....
    if (downKey.key === 'ArrowLeft') { prevButton(); }
    if (downKey.key === 'ArrowRight') { nextButton(); }
})

document.addEventListener('keydown', function (downKey) {
    if (downKey.key === 'Tab') return;
    if (!lightbox.open) return;
    const focusables = lightbox.querySelectorAll('button, [href], input, select, textarea, [tabindex]: not([tabindex="-1"])');
    // standart paste&copy sry
    const listOfFocusables = Array.from(focusables);
    if (!listOfFocusables.length) return;
    // wenns nix zu tabben gibt
    const firstOfFocusables = listOfFocusables[0];
    const lastOfFocusables = listOfFocusables[listOfFocusables.length - 1];
    ///////////  Tab-Karussel (painInTheAss) 
    if (downKey.shiftKey && document.activeElement === firstOfFocusables) {
        downKey.preventDefault();
        // tab springt nicht ins Nirvana
        lastOfFocusables.focus();
    } else
        if (!shiftKey && document.activeElement === lastOfFocusables) {
            downKey.preventDefault();
            firstOfFocusables.focus;
        }
    //////////
});
          ...
          <figure>
            <img src="${thumbPath(img.src)}"
              ...
              onkeydown="if (event.key === 'Enter' || event.key === ' ') 
                { event.preventDefault(); openByIndex(${i}); }">
...
function openByIndex(i) {
    ...
    // für tabbing merken
    lightbox.showModal();
    // blockiert alles ausser dialog
    document.getElementById('next_btn').focus();
    // setzt tabbing auf next_button
...
renderGallery();
// funktionsaufruf um die thumbs zu rendern
